...
export default function UserManagementPage() {
    ...
    const roleBadge = (role: string) => {
        ...
        return <span className={`px-2 py-0.5 rounded-full text-[11px] font-bold uppercase tracking-wide ${styles[role] || styles.CREW}`}>{role.replace('_', ' ')}</span>;
    };
            ...
            <PullToRefresh onRefresh={fetchUsers}>
                <div className="bg-white dark:bg-[#1c1c1e] rounded-2xl shadow-sm border border-gray-200 dark:border-gray-800 overflow-hidden">
                    ...
                    <div className="hidden sm:flex items-center px-5 py-3 border-b border-gray-100 dark:border-gray-800 text-xs font-bold uppercase tracking-wider text-gray-500 dark:text-gray-400">
                        ...
                        <button className="flex items-center gap-1 w-[120px] hover:text-gray-700 dark:hover:text-gray-200 transition-colors" onClick={() => handleSort('role')}>
                        ...
                        <button className="flex items-center gap-1 w-[140px] hover:text-gray-700 dark:hover:text-gray-200 transition-colors" onClick={() => handleSort('department')}>
                            Department <SortIcon active={sortKey === 'department'} dir={sortKey === 'department' ? sortDir : 'asc'} />
                        </button>
                        <div className="hidden lg:block w-[340px] shrink-0 text-right pr-2">
                            Actions
                        </div>
                    </div>
                                ...
                                >
                                    ...
                                    <div className="hidden sm:flex flex-col lg:flex-row lg:items-center justify-between">
                                        <div className="flex items-center flex-1 min-w-0">
                                            ...
                                            {/* Role */}
                                            <div className="w-[120px] shrink-0">{roleBadge(u.role)}</div>

                                            {/* Status */}
                                            <div className="w-[110px] shrink-0">{statusBadge(u.status)}</div>

                                            {/* Department */}
                                            <div className="w-[140px] shrink-0 text-xs text-gray-600 dark:text-gray-300 font-medium truncate">
                                                {getDeptName(u.departmentId)}
                                            </div>
                                        </div>

                                        {/* Actions Row */}
                                        <div className="hidden sm:flex items-center gap-2 mt-3 ml-[52px] lg:mt-0 lg:ml-4 lg:w-[340px] lg:justify-end shrink-0">
                                        {user?.id !== u.id ? (
                                            <>
                                            {/* Role Changer */}
                                            <select
                                                className="bg-gray-100 dark:bg-gray-800 border-0 rounded-lg px-2.5 py-1.5 text-xs font-medium cursor-pointer focus:ring-2 focus:ring-blue-500 text-gray-700 dark:text-gray-300"
                                                value={u.role}
                                    ...
                                    <div className="sm:hidden space-y-3">
                                        ...
                                        {user?.id !== u.id && (
                                            <div className="flex gap-1.5">
                                                <select
                                                    className="flex-1 bg-gray-100 dark:bg-gray-800 border-0 rounded-xl px-3 py-2.5 text-xs font-semibold focus:ring-2 focus:ring-blue-500 text-gray-700 dark:text-gray-300"
                                                    value={u.role}
                                                    onChange={(e) => handleRoleChange(u.id, e.target.value)}
                                                >
                                                    <option value="CREW">Crew</option>
                                                ...
                                                <button
                                                    onClick={() => handleToggleStatus(u.id, u.status)}
                                                    className={`px-3 py-2.5 rounded-xl text-xs font-semibold transition-all active:scale-95 ${u.status === 'ACTIVE'
